...
class AnomalyCLIP_PromptLearner(nn.Module):
    def __init__(self, clip_model, design_details, classnames=None, state_list=None):
        super().__init__()
        ...
        # self.state_normal_list = ["{} with all parts intact", ]
        if state_list is None:
            self.state_normal_list = ["{} with all parts flawless", ]
            self.state_anomaly_list = ["{} with damaged parts", ]
            # self.state_normal_list = ["the all partially intact {}", ]
            # self.state_anomaly_list = ["the partially damaged {}", ]
        else:
            self.state_normal_list = [state_list[0]]
            self.state_anomaly_list = [state_list[1]]
