...
def _download(
    ...
    if not cache_dir:
        ...
        cache_dir = os.path.expanduser("//home/worker1/CLIP-main/.cache/clip")
...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", design_details = None, jit: bool = False, download_root: str = None):
    ...
    if name in _MODELS:
        ...
        model_path = _download(_MODELS[name], download_root or os.path.expanduser("/home/worker1/AD_CLIP/.cache/clip"))
...
def get_similarity_map(sm, shape):
    ...
    # plt.figure()
    # plt.imshow(sm.cpu().numpy()[0][0])
    # plt.show()

    sm = torch.nn.functional.interpolate(sm, shape, mode='bilinear')
    # sm = torch.nn.ConvTranspose2d(2, 2, 3, 14, output_padding=11, groups=2).to(0)(sm)
    # from .deform_conv import DeformConv2d
    # sm_dc = DeformConv2d(2, 2, 3, 1, 1, groups=2).to(0)(sm)
    #
    # plt.subplot(1, 2, 1)
    # plt.imshow(sm.cpu().numpy()[0][0])
    # plt.subplot(1, 2, 2)
    # plt.imshow(sm_dc.cpu().numpy()[0][0])
    # plt.show()
...
def get_similarity_map_conv(sm, layer, shape):
    side = int(sm.shape[1] ** 0.5)
    sm = sm.reshape(sm.shape[0], side, side, -1).permute(0, 3, 1, 2)
    sm = torch.nn.functional.interpolate(sm, shape, mode='bilinear')
    sm = layer(sm)
...
#     feats = image_features.unsqueeze(1)[:, :, :1, :] * text_features
#     similarity = feats.sum(-1)
