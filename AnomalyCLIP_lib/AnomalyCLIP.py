...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        cov = torch_cov(q)
        attn = (q @ torch.inverse(cov) @ k.transpose(-2, -1)) * self.scale
...
class ResidualAttentionBlock_learnable_token(nn.Module):
    ...
    def forward(self, inputs, compound_prompt_nctx=None):
        if compound_prompt_nctx is None:
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, out_layers = [6, 12, 18, 24], DPAM_layer = None, ffn = False, compound_prompt_nctx=None):
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, features_list, ori_patch = False, proj_use = True, DPAM_layer = None, ffn = False):
        ...
        if True:
            ...
            return x_ori[0, :, :] @ self.proj, patch_tokens, x_ori @ self.proj
...
class AnomalyCLIP(nn.Module):
    ...
    def encode_text_learn(self, prompts, tokenized_prompts, deep_compound_prompts_text = None, normalize: bool = False, compound_prompt_nctx=None):
        ...
        if deep_compound_prompts_text is None:
            ...
            x = self.transformer([x, deep_compound_prompts_text, 0], compound_prompt_nctx=compound_prompt_nctx)
        x = x.permute(1, 0, 2)  # LND -> NLD
        x = self.ln_final(x).type(self.dtype)  # [batch_size, n_ctx, transformer.width]
        # take features from the eot embedding (eot_token is the highest number in each sequence)
        x_out = x[torch.arange(x.shape[0]), tokenized_prompts.argmax(dim=-1)] @ self.text_projection
        return x_out.float(), (x @ self.text_projection).float()
